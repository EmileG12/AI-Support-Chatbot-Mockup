...
Rendered inside [StaffDashboard](StaffDashboard.md), above the ticket list. Shows customers
currently waiting for a staff member (see the [working-hours handoff](../backend-services/conversationFlow.md))
...
- Clicking "Join" calls [staffJoin](../frontend-utils/staffJoin.md) for that conversation (button
  ...
  conversation id and the `{ draftTicket, messages }` result — [StaffDashboard](StaffDashboard.md)
  owns what happens next (opening [StaffChatWindow](StaffChatWindow.md)), this component doesn't
  render the chat itself.
- A failed join shows an inline error banner.

## Related

- [docs/components/StaffDashboard.md](StaffDashboard.md), [docs/components/StaffChatWindow.md](StaffChatWindow.md)
