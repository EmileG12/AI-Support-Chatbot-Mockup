...
Defined in `supabase/migrations/20260923000000_init_chat_tickets.sql`, then altered by `20260923010000_add_landline_fault_category.sql` (added `landline_fault` to the category check), `20260923020000_add_troubleshooting_notes.sql` (added `troubleshooting_notes`), and `20260923030000_add_duplicate_detection.sql` (added `possible_duplicate_of`, `duplicate_similarity`, and a trigram index).

## Purpose

One row per logged support ticket: the AI's classification (`category`, `priority`, `summary`), what it captured from the customer (`raw_message`, `customer_name`, `customer_contact`), troubleshooting diagnostics already covered, its lifecycle `status`, and any duplicate-ticket flag.
...
- `possible_duplicate_of` / `duplicate_similarity` — set by [docs/api-routes/post-api-chat.md](../api-routes/post-api-chat.md) via [find_possible_duplicate_ticket](../rpc-functions/find_possible_duplicate_ticket.md) at insert time; never updated afterward.
...
- [docs/api-routes/README.md](../api-routes/README.md) (all four ticket routes touch this table)
