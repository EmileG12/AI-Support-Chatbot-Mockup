...
Server-side validation of customer contact details (name/email/phone), used regardless of where
the data came from — an LLM tool call or a customer-typed correction form — consistent with the
security-minded posture the rest of the backend takes (e.g. `PATCH /api/tickets/:id`'s
category/priority/status validation).
...
naming the first failing field. Checks: `name` non-blank after trimming; `email` matches a basic
`local@domain.tld` pattern; `phone` matches a loose pattern allowing `+`, digits, spaces,
parentheses and hyphens, 7–20 characters.
